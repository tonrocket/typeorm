...
const TableCheck_1 = require("../../../src/schema-builder/table/TableCheck");
...
describe("query runner > create check constraint", () => {
    ...
    it("should correctly create check constraint and revert creation", () => Promise.all(connections.map(async (connection) => {
        ...
        const check1 = new TableCheck_1.TableCheck({
            expression: `${driver.escape("name")} <> 'asd' AND ${driver.escape("description")} <> 'test'`,
        });
        const check2 = new TableCheck_1.TableCheck({
            expression: `(${driver.escape("id")} < 0 AND ${driver.escape("version")} < 9999) OR (${driver.escape("id")} > 9999 AND ${driver.escape("version")} < 888)`,
        });
        const check3 = new TableCheck_1.TableCheck({
