...
describe("database schema > generated columns > oracle", () => {
    ...
    it("should create table with generated columns", () => Promise.all(dataSources.map(async (dataSource) => {
        ...
        virtualFullName.asExpression.should.be.equal(`CONCAT("firstName", "lastName")`);
        ...
        nameHash.asExpression.should.be.equal(`standard_hash(coalesce("firstName",'MD5'))`);
    ...
    it("should add generated column and revert add", () => Promise.all(dataSources.map(async (dataSource) => {
        ...
        virtualColumn.asExpression.should.be.equal(`"firstName" || '_' || "lastName"`);
        // revert changes
        ...
        const metadataRecords = await queryRunner.query(`SELECT * FROM "typeorm_metadata" WHERE "table" = 'post' AND "name" = 'virtualColumn'`);
    ...
    it("should drop generated column and revert drop", () => Promise.all(dataSources.map(async (dataSource) => {
        ...
        (0, chai_1.expect)(table.findColumnByName("virtualFullName")).to.be
            .undefined;
        // check if generated column records removed from typeorm_metadata table
        const metadataRecords = await queryRunner.query(`SELECT * FROM "typeorm_metadata" WHERE "table" = 'post' AND "name" = 'virtualFullName'`);
        metadataRecords.length.should.be.equal(0);
        // revert changes
        await queryRunner.executeMemoryDownSql();
        table = await queryRunner.getTable("post");
    ...
    it("should change generated column and revert change", () => Promise.all(dataSources.map(async (dataSource) => {
        const queryRunner = dataSource.createQueryRunner();
        let table = await queryRunner.getTable("post");
        let virtualFullName = table.findColumnByName("virtualFullName");
        ...
        virtualFullName.asExpression.should.be.equal(`'Mr.' || "firstName" || ' ' || "lastName"`);
...
//# sourceMappingURL=generated-columns-oracle.js.map