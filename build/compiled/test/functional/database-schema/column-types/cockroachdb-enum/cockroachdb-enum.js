...
describe("database schema > column types > cockroachdb-enum", () => {
    ...
    it("should change `enumName` and revert change", () => Promise.all(connections.map(async (connection) => {
        ...
        (0, chai_1.expect)(changedColumn.enumName).to.equal("PostTypeEnum");
        // revert changes
        await queryRunner.executeMemoryDownSql();
    ...
    it("should change both ENUM value and ENUM name and revert change", () => Promise.all(connections.map(async (connection) => {
        ...
        (0, chai_1.expect)(columnAfterRevert.enumName).to.undefined;
        await queryRunner.release();
    })));
...
//# sourceMappingURL=cockroachdb-enum.js.map