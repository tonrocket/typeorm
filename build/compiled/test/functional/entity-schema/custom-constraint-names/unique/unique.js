...
describe("database schema > custom constraint names > unique", () => {
    ...
    it("should load constraints with custom names", () => Promise.all(dataSources.map(async (dataSource) => {
        ...
        if (DriverUtils_1.DriverUtils.isMySQLFamily(dataSource.driver) ||
            ...
            dataSource.driver.options.type === "sap" ||
            dataSource.driver.options.type === "spanner") {
            const uniqueIndex = table.indices.find((it) => it.name === "UQ_NAME");
            (0, chai_1.expect)(uniqueIndex).to.exist;
        }
        else {
