    ...
    describe("inverse side", () => {
        it("should insert, load, update and remove entities with embeddeds when embedded entity having ManyToMany relation", () => Promise.all(connections.map(async (connection) => {
            ...
            const loadedUser2 = await connection.manager
                ...
                .where("user.id = :id", { id: 1 })
                .getOne();
            (0, chai_1.expect)(loadedUser2.should.be.eql({
                id: 1,
                name: "Anna",
                likedPosts: [
                    {
            ...
            await connection.getRepository(User_1.User).remove(loadedUser2);
            const loadedUsers2 = (await connection
                .getRepository(User_1.User)
                .find({ order: { name: "ASC" } }));
            (0, chai_1.expect)(loadedUsers2.length).to.be.equal(2);
            (0, chai_1.expect)(loadedUsers2[0].name).to.be.equal("Bob");
            (0, chai_1.expect)(loadedUsers2[1].name).to.be.equal("Clara");
        })));
    });
});
//# sourceMappingURL=embedded-many-to-many-case1.js.map