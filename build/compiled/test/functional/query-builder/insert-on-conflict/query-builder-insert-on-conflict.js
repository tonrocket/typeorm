...
describe("query builder > insertion > on conflict", () => {
    ...
    it("should perform insertion on partial index using orUpdate", () => Promise.all(connections.map(async (connection) => {
        if (connection.driver.options.type !== "postgres")
            return;
        const post1 = new Post_1.Post();
        post1.id = "post#1";
        ...
        const sql = connection.manager
            ...
            .disableEscaping()
            .getSql();
        (0, chai_1.expect)(sql).to.equal(`INSERT INTO post(id, title, date) ` +
            `VALUES ($1, $2, $3) ON CONFLICT ( date ) ` +
            `WHERE ( date > 2020-01-01 ) DO UPDATE SET title = EXCLUDED.title`);
    })));
    it("should perform insertion using partial index and skipping update on no change", () => Promise.all(connections.map(async (connection) => {
        if (connection.driver.options.type !== "postgres")
            return;
        const post1 = new Post_1.Post();
        post1.id = "post#1";
    ...
    it("should throw error if using indexPredicate amd an unsupported driver", () => Promise.all(connections.map(async (connection) => {
        if (!connection.driver.supportedUpsertTypes.includes("on-duplicate-key-update"))
            return;
        const post1 = new Post_1.Post();
        post1.id = "post#1";
        post1.title = "About post";
        ...
        const sql = connection.manager
            ...
            .disableEscaping()
            .getSql();
        (0, chai_1.expect)(sql).to.throw(Error);
    })));
});
